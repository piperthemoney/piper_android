...
object MmkvManager {
    ...
    fun decodeAssetUrls(): List<Pair<String, com.piperbloom.proxyvpn.dto.AssetUrlItem>> {
        val assetUrlItems = mutableListOf<Pair<String, com.piperbloom.proxyvpn.dto.AssetUrlItem>>()
        assetStorage?.allKeys()?.forEach { key ->
            val json = assetStorage?.decodeString(key)
            if (!json.isNullOrBlank()) {
                assetUrlItems.add(Pair(key, Gson().fromJson(json, com.piperbloom.proxyvpn.dto.AssetUrlItem::class.java)))
            }
        }
