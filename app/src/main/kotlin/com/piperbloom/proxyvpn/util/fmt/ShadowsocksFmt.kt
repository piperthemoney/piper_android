...
object ShadowsocksFmt {
    fun parseShadowsocks(str: String): ServerConfig? {
        val config = ServerConfig.create(com.piperbloom.proxyvpn.dto.EConfigType.SHADOWSOCKS)
        if (!tryResolveResolveSip002(str, config)) {
            var result = str.replace(com.piperbloom.proxyvpn.dto.EConfigType.SHADOWSOCKS.protocolScheme, "")
